...
.config(['$analyticsProvider', '$windowProvider', function ($analyticsProvider, $windowProvider) {
   ...
   */
  $analyticsProvider.registerSetUserProperties(function (properties, customMap) {

    if (properties) {
      dispatchToGa('setUserProperties', 'set', dimensionsAndMetrics(properties, customMap));
    }
   ...
   */
  function dimensionsAndMetrics(properties, customMap) {
    // add custom dimensions and metrics
    var customData = {};
    var key;
    var onlyDimension = properties['no_metric'] ? true : false;
    ...
    for (key in properties) {
      ...
      if(customMap[key]) {
      	...
      	var mapData = customMap[key];
      	
				if(!onlyDimension || (onlyDimension && mapData.type == 'dimension')) customData[mapData.type+mapData.index] = properties[key].toString();
