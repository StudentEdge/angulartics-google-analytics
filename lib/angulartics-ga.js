...
.config(['$analyticsProvider', '$windowProvider', function ($analyticsProvider, $windowProvider) {
  ...
  $analyticsProvider.registerPageTrack(function (path, properties) {
    ...
    dispatchToGa($analyticsProvider.settings.ga.token, 'config', angular.extend(angular.copy(properties), {
      page_path: path
    }));
  ...
  $analyticsProvider.registerEventTrack(function(action, properties) {
    ...
    dispatchToGa(action, 'event', angular.extend(angular.copy(properties), {
      hitType: 'event',
  ...
  $analyticsProvider.registerSetUserProperties(function (properties) {
    ...
    if (properties) {
    	persistentValues = angular.copy(properties);
      dispatchToGa($analyticsProvider.settings.ga.token, 'config', {
      	custom_map: dimensionsAndMetrics(formatDimensionsAndMetrics(properties))
   		});
    }
  ...
  $analyticsProvider.registerUserTimings(function (properties) {
    ...
    dispatchToGa('timing_complete', 'event', {
      hitType: 'timing',
      event_category: properties.timingCategory,
      name: properties.timingVar,
      value: properties.timingValue,
      event_label: properties.timingLabel,
   ...
   * @name formatDimensionsAndMetrics
   * 
   ...
   */
  function formatDimensionsAndMetrics(properties) {
    // add format dimensions and metrics
    var formatData = {};
    var key;
    var i = 1;
    ...
    for (key in properties) {
      ...
      formatData['dimension'+i] = key;
      //formatData['metric'+i] = properties[key];
      i++;
    }
  ...
  function dispatchToUniversal_(command, fieldsObj, shouldCopyHit) {
    ...
    if (userId && angular.isObject(fieldsObj)) fieldsObj.user_id = userId;
    ...
    if (command.indexOf('ecommerce:') > -1 && $analyticsProvider.settings.ga.enhancedEcommerce) {
      ...
      switch (command) {
        ...
        case 'ecommerce:send':
          ...
          fieldsObj.event_category = 'Angulartics Enhanced Ecommerce';
		...
		var method = fieldsObj.method ; 
		
		delete fieldsObj.method;
		
		if(!fieldsObj.custom_map) fieldsObj = angular.extend({}, persistentValues, fieldsObj);
    
    uaCommand = command instanceof Array ? [method, command.concat(fieldsObj)] : [command, method, fieldsObj];
  ...
  function applyUniversalCall_(commandArray, shouldCopyHit) {
    ...
    var userId = $analyticsProvider.settings.ga.userId;
    var gaNamespace = 'gtag'; // $window.GoogleAnalyticsObject;
    var commandClone;
